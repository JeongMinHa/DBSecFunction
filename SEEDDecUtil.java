...
/*
 * SEED를 이용한 복호화 처리 유틸 클래스
 */
public class SEEDDecUtil {
	
	public static void main(String[] args){
		decValReturn(args[0]);
	}
	
	public static String decValReturn(String sDecValue){
		
		String rtrnValue = null;
		
		try{
			int[] seedKey = SEEDDecUtil.getSeedRoundKey("*************");
			
			rtrnValue = SEEDDecUtil.getDecrypt(sDecValue , seedKey);
	...
	//암,복호화된 문자열 처리 시 사용할 언어셋
	private static final String sCharSet = "KSC5601";
	
	//XOR 연산 시 사용할 일종의 키값 
	private static final String sDefaultIV = "isecommerce.co.ltd";
	
	/*
	 * 평문 문자열을 암호화
	 */
	public static String getEncrypt(String sPlainValue, int[] iSeedRoundKey) throws Exception {
		
		//평문 문자열을  byte형의 배열 리스트로 변환
		//List<byte[]> plainByteLists = getStrToList(sPlainValue, false);
		List plainByteLists = getStrToList(sPlainValue, false);

		//암호화될 byte형의 배열 리스트 선언
		//List<byte[]> encByteLists = new ArrayList<byte[]>();
		List encByteLists = new ArrayList();

		// IV를 저장하라 변수 선언
		byte[] byteIV = sDefaultIV.getBytes();

		for (int i = 0; i < plainByteLists.size(); i++) {

			byte[] byteVal = (byte[]) plainByteLists.get(i);
			
			byte[] tempEncVal = new byte[16];

			//블록암호 운영모드에는 ECB, CBC, CFB, OFB, CTR 와 같이 있는데, SEED에서는 CBC 모드를 택하고 있다.
			byteVal = exclusiveOR(byteVal, byteIV);

			tempEncVal = SEEDDecUtil.SeedEncrypt(byteVal, iSeedRoundKey, tempEncVal);

			encByteLists.add(tempEncVal);
		}

		//List에 담긴 암호화문을 String 형으로 변환
		return getByteToListStr(encByteLists, true);
	}
	
	/*
	 * 암호화된 문자열을 복호화하는 메소드
	 */
	public static String getDecrypt(String sEncValue, int[] iSeedRoundKey) throws Exception {
		
		//암호화된 문자열을 byte형의 배열 리스트로 변환
		//List<byte[]> encByteLists = getStrToList(sEncValue, true);
		List encByteLists = getStrToList(sEncValue, true);
		
		//복호화될 byte형의 배열 리스트 선언
		//List<byte[]> decByteList = new ArrayList<byte[]>();
		List decByteList = new ArrayList();
		
		//IV를 byte형으로 저장할 변수 선언
		byte[] byteIV = sDefaultIV.getBytes();
		
		for(int i=0; i < encByteLists.size(); i++){
			byte[] encByte = (byte[])encByteLists.get(i);
			
			byte[] tempDecByte = new byte[16];
			
			tempDecByte = SEEDDecUtil.SeedDecrypt(encByte, iSeedRoundKey, tempDecByte);
			
			//Encryption 에서 CBC모드로 연산했으니 Decryption 할 때도 CBC 모드로 연산  
			tempDecByte = exclusiveOR(tempDecByte, byteIV);
			
			decByteList.add(tempDecByte);
		}
		
		//List에 담긴 암호화문을 String 형으로 변환
		return getByteToListStr(decByteList, false);
	}
	
	/*
	 *암호화 또는 복호화 할 때 사용할 SEED RoundKey 생성.
	 */
	public static int[] getSeedRoundKey(String keyStr) throws Exception {
		int[] seedKey = new int[32];
		
		SEEDDecUtil.SeedRoundKey(seedKey, keyStr.getBytes());
		
		return seedKey;
	}
	
	/*
	 *CBC 모드를 사용하기 위한 XOR 메소드
	 */
	private static byte[] exclusiveOR(byte[] value1, byte[] value2) {
		for(int i=0; i<16; i++){
	...
	/*
	 *SEED는 128bit 블록 단위로 암호화 처리하므로,
	 *문자열을 128bit 단위로 byte 배열을 생성하고,
	 *그 배열을 List형으로 묶어서 반환
	 */
	//private static List<byte[]> getStrToList(String sSeedUtilVal, boolean isDecode) throws Exception {
	private static List getStrToList(String sSeedUtilVal, boolean isDecode) throws Exception {
		
		//List<byte[]> byteLists = new ArrayList<byte[]>();
		List byteLists = new ArrayList();
		
		byte[] tempByte = null;
		
		if(isDecode){ //암호화문의 경우
			BASE64Decoder base64Dec = new BASE64Decoder();
			
			tempByte = base64Dec.decodeBuffer(sSeedUtilVal);
		}else{ //평문의 경우
			tempByte = sSeedUtilVal.getBytes(sCharSet);
		}
		
		//암호화를 위해선 byte가 무조건 16byte씩 배열이 생성되어야 함으로 빈 배열의 공간 수를 계산
		int iNeedBlankLength = 0;
		
		if(tempByte.length % 16 != 0){
			iNeedBlankLength = 16 - (tempByte.length % 16);
		}
		
		//위에서 구한 필요한 빈 공간의 수를 더하여 다시 배열 생성
		byte[] newTempByte = new byte[tempByte.length + iNeedBlankLength];
		
		for(int i=0; i < tempByte.length; i++){
			newTempByte[i] = tempByte[i];
		}
		
		//이제 newTempByte를 16의 배수의 사이즈를 갖는 배열이 되었으므로 16개씩 짤라서 List에 add 한다.
		int iListByteIdx = 0;
		...
		for(int i=0; i<newTempByte.length; i++){
			...
			byteList[iListByteIdx] = newTempByte[i];
	...
	/*
	 * 128bit단위의  byte 배열이 담긴 List를 문자열로 변환.	 
	 */
	private static String getByteToListStr(List listByteValue, boolean isEncode) throws Exception {

		String retStr = null;
		
		// List에 담긴 byte배열이 16으로 고정이므로 * 16
		byte[] byteList = new byte[listByteValue.size() * 16];

		// List에 담긴 byte배열을 하나의 배열(byteList)에 추가
		for (int i = 0; i < listByteValue.size(); i++) {
			
			byte[] temp = (byte[]) listByteValue.get(i);

			for (int j = 0; j < temp.length; j++) {
				byteList[j + (16 * i)] = temp[j];
			}
		}
		...
		for (int i = byteList.length; i > 0; i--) {
		...
		byte[] byteResult = new byte[byteList.length - blankCnt];
		
		for (int i = 0; i < byteResult.length; i++) {
			byteResult[i] = byteList[i];
		}
		
		if (isEncode) {
			BASE64Encoder base64Enc = new BASE64Encoder();
			retStr = base64Enc.encode(byteResult);
		} else {
			retStr = new String(byteResult, sCharSet);
		}
	...
	private static int SS0[] = {
		...
		// Java virtual machine uses big endian as a default
		...
		public static byte[] SeedEncrypt(byte pbData[], // [in] data to be encrypted
		...
		public static byte[] SeedDecrypt(byte pbData[], // [in] encrypted data
